...
def main():
    ...
    with tab1:
        with st.container():
            ...
            if st.session_state.res_t1:
                ...
                # Recommandations (partielles) – les recos complètes sont dans l'onglet 2
    ...
    with tab2:
        with st.container():
            ...
            with col2:
                ...
                # Champ tension source
                source_voltage = st.number_input(
                    "Tension au départ (source) [V]",
                    min_value=100.0,
                    value=None,
                    step=10.0,
                    help="Laissez vide pour utiliser la tension du réseau sélectionné"
                )
                if source_voltage is not None and source_voltage > 0:
                    tension_ref = source_voltage
                elif network_type != "Personnalisé":
                    tension_ref = Constants.NETWORK_VOLTAGES[network_type]
                else:
                    tension_ref = st.number_input("Tension (V)", min_value=100.0, value=400.0, step=10.0, key="custom_voltage")
            ...
            if st.session_state.res_t2:
                ...
                # Détail par phase avec mise en évidence
                st.markdown("#### Détail par phase")
                cols = st.columns(3)
                for idx, col in enumerate(cols):
                    ph = r['phases'][idx]
                ...
                # Recommandations
                all_recs = generate_recommendations(st.session_state.res_t1, r)
                if all_recs:
                ...
                # Données pour PDF
                data_rows = [
                    ("Tension source", f"{tension_ref:.0f}", "V"),
    ...
    <div class="onee-footer">
        <span class="badge">ONEE Tech v2.7</span>
