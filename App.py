...
# --- PDF Generation ---
def generate_pdf_report(title: str, data_rows: List[Tuple[str, str, str]], summary_message: str, client_name: str = "") -> io.BytesIO:
...
# --- Fonctions de calcul améliorées pour la charge ---
def calculate_transformer_load(
    s_nom: float,
    p_phases: Tuple[float, float, float],
    ...
    """
    Calcule la charge du transformateur avec :
    - cos φ par phase
    - facteur de simultanéité
    - déclassement par température
    - puissance future
    """
    # Puissance active totale avec facteur de simultanéité
    p_active_total = sum(p_phases) * simultaneity

    # Puissance apparente par phase
    s_phases = [p / cos if cos > 0 else 0 for p, cos in zip(p_phases, cos_phases)]
    s_total = sum(s_phases) * simultaneity  # on applique aussi simultanéité sur S
    ...
    # Taux de charge actuel (par rapport à la puissance nominale déclassée)
    charge_actual = (s_total / s_nom_derated) * 100 if s_nom_derated > 0 else 0

    # Taux de charge futur (si future_p > 0)
    if future_p > 0:
        # On suppose que le facteur de puissance global reste identique à l'actuel
...
def suggest_cable_section(s_total_kva, tension_v=400, L_m=100, material="Cuivre (Cu)"):
    """Suggère une section de câble minimale pour une puissance donnée."""
    ...
    # R = rho * L / S
    # ΔU = sqrt(3) * I * R * cosφ  (on prend cosφ = 0.9)
    # S_min = (sqrt(3) * I * rho * L * cosφ) / delta_u_max
    cosphi = Constants.COS_PHI
    S_min = (math.sqrt(3) * I * rho * L_m * cosphi) / delta_u_max
    # Arrondir à la section standard supérieure
...
def main():
    ...
    # ===================== TAB 1 : CHARGE TRANSFORMATEUR (amélioré) =====================
    with tab1:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
            st.markdown('<div class="card-title">🔌 Charge Transformateur (avec déséquilibre et déclassement)</div>', unsafe_allow_html=True)

            # Données nominales
            col1, col2 = st.columns(2)
            with col1:
                s_nom = st.number_input("Puissance nominale du transformateur (kVA)", min_value=1.0, value=100.0, step=10.0)
            with col2:
            ...
            default_p = [45.0, 55.0, 35.0]
            p_vals = []
            for idx, (col, label, default) in enumerate(zip(cols, phase_labels, default_p)):
                with col:
                    p = st.number_input(label, min_value=0.0, value=default, step=5.0, key=f"p_{idx}")
                    p_vals.append(p)
            ...
            if st.button("Calculer la charge", key="btn_t1", use_container_width=True):
                result = calculate_transformer_load(
                    s_nom, tuple(p_vals), tuple(cos_vals), simultaneity, temp_amb, future_p
                )
            ...
            if st.session_state.res_t1:
                ...
                if cable_length > 0:
                    recommended_section = suggest_cable_section(r['s_total'], tension_v=400, L_m=cable_length, material="Cuivre (Cu)")
                ...
                # Ajouter les puissances par phase
                for i in range(3):
                ...
                if REPORTLAB_AVAILABLE:
                    pdf_buffer = generate_pdf_report("Rapport Charge Transformateur", data_rows, r['message'], client_name="")
                    if pdf_buffer:
                        st.download_button(
    ...
    with tab2:
        with st.container():
            ...
            if st.session_state.res_t2:
                ...
                if REPORTLAB_AVAILABLE:
                    pdf_buffer = generate_pdf_report("Rapport Chute de Tension", data_rows, r['message'], client_name=st.session_state.client_name)
                    if pdf_buffer:
                        st.download_button(
                            label="📄 Télécharger le rapport PDF",
                            data=pdf_buffer,
