...
def init_session_state():
    for key in ["res_t1", "res_t2", "client_name", "poste_name", "poste_matricule"]:
        if key not in st.session_state:
            st.session_state[key] = None if "res" in key else ""

init_session_state()

# --- CSS amélioré pour les indicateurs et couleurs ---
def load_css():
    st.markdown("""
    <style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    /* Highlight for out-of-limit metrics */
    .metric-critical {
...
# --- Fonctions de calcul (inchangées, mais avec les constantes corrigées) ---
def calculate_transformer_load(
    ...
    for I in currents:
        ...
        q_phases.append(q)
        s_phases.append(s)
    p_total = sum(p_phases) * simultaneity
    q_total = sum(q_phases) * simultaneity
    ...
    return {
        "s_nom": s_nom, "s_nom_derated": s_nom_derated, "derate_factor": derate_factor,
        "currents": currents, "p_phases": p_phases, "q_phases": q_phases, "s_phases": s_phases,
        "cos_phi": cos_phi, "p_active_total": p_total, "q_total": q_total, "s_total": s_total,
        "cos_global": cos_global, "charge_actual": charge_actual, "future_p": future_p,
        "charge_future": charge_future, "status": status, "message": message,
        "message_future": message_future, "simultaneity": simultaneity,
        "temp_amb": temp_amb, "line_voltage": line_voltage
    }
...
def display_compliance_indicator(status: str, text: str):
    colors = {"ok": "green", "warn": "orange", "err": "red"}
    st.markdown(f'<div style="background:{colors[status]}; color:white; padding:8px 12px; border-radius:8px; display:inline-block; font-weight:bold;">{text}</div>', unsafe_allow_html=True)

def plot_voltage_drop_bar_chart(drops: List[float], phases: List[int]):
    ...
    ax.set_xlabel('Phase')
    ax.set_title('Chutes de tension par phase')
...
def generate_recommendations(load_result: Dict, drop_result: Dict) -> List[str]:
    recs = []
    if load_result:
        ...
        # Déséquilibre de courant (calculé dans drop_result)
    if drop_result:
        imbalance = drop_result['imbalance']['max_deviation']
        if imbalance > Constants.UNBALANCE_WARNING:
            recs.append(f"⚖️ DÉSÉQUILIBRE DE PHASES ({imbalance:.1f}%) : Répartissez les charges entre les phases.")
        max_drop = drop_result['max_drop']
        if max_drop > Constants.VOLTAGE_DROP_CRITICAL:
            recs.append(f"📉 CHUTE DE TENSION EXCESSIVE ({max_drop:.2f}%) : Augmentez la section du câble ou réduisez la distance.")
        elif max_drop > Constants.VOLTAGE_DROP_WARNING:
            recs.append(f"⚠️ CHUTE DE TENSION LIMITE ({max_drop:.2f}%) : Vérifiez le dimensionnement du câble.")
        if drop_result['recommendations']:
            recs.extend(drop_result['recommendations'])
    return recs
...
def main():
    ...
    with tab1:
        with st.container():
            ...
            st.markdown('<div class="card-title">🔌 Charge Transformateur (saisie par courant, cos φ = 0.8)</div>', unsafe_allow_html=True)
            ...
            simultaneity = st.slider("Facteur de simultanéité", 0.5, 1.0, 1.0, 0.01)
            ...
            if st.button("Calculer la charge", key="btn_t1", use_container_width=True):
                result = calculate_transformer_load(s_nom, tuple(currents), line_voltage, simultaneity, temp_amb, future_p)
                st.session_state.res_t1 = result

            if st.session_state.res_t1:
                r = st.session_state.res_t1

                # Indicateur de conformité clair (3)
                ...
                # Mise en évidence des valeurs hors normes (11)
                metric_class = "metric-warning" if r['charge_actual'] > Constants.CHARGE_WARNING else "metric-ok"
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Puissance active totale", f"{r['p_active_total']:.1f} kW")
                with col2:
                    st.metric("Puissance apparente totale", f"{r['s_total']:.1f} kVA")
                with col3:
                    st.metric("cos φ global", f"{r['cos_global']:.2f}")
                with col4:
                    st.metric("Facteur de déclassement", f"{r['derate_factor']:.3f}")
                ...
                for idx, col in enumerate(cols):
                    with col:
                        ...
                        <div style="background:#F8F9F8; border-radius:12px; padding:12px; margin:4px 0; border-left:4px solid #{'D32F2F' if idx==0 else '1B5E20'}">
                            ...
                            Q réactive: {r['q_phases'][idx]:.1f} kVAR<br/>
                ...
                # Recommandations dynamiques (6) – pour le moment on appelle la fonction après les deux calculs
                # Mais on peut afficher ici les recos liées à la charge
                ...
                # Préparation des données pour PDF (inchangé)
                data_rows = [
                    ...
                    ("cos φ global", f"{r['cos_global']:.2f}", ""),
                ...
                for i in range(3):
                    ...
                    data_rows.append((f"Q réactive phase {i+1}", f"{r['q_phases'][i]:.1f}", "kVAR"))
                    data_rows.append((f"S phase {i+1}", f"{r['s_phases'][i]:.1f}", "kVA"))

                if REPORTLAB_AVAILABLE:
                    pdf_buffer = generate_pdf_report("Rapport Charge Transformateur", data_rows, r['message'],
    ...
    with tab2:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
            st.markdown('<div class="card-title">📉 Chute de Tension (cos φ = 0.9)</div>', unsafe_allow_html=True)
            ...
            if st.button("Calculer la chute de tension", key="btn_t2", use_container_width=True):
                st.session_state.res_t2 = calculate_three_phase_unbalanced(tuple(currents), L, section, material, tension_ref)

            if st.session_state.res_t2:
                r = st.session_state.res_t2

                # Indicateur de conformité clair (3)
                status, status_text = get_drop_status(r['max_drop'])
                st.markdown(f'<div style="display: flex; justify-content: space-between; align-items: center;">'
                            f'<div><span style="font-size: 24px; font-weight: bold;">Conformité chute de tension</span></div>'
                            f'<div style="background: {"#4CAF50" if status=="ok" else "#FF9800" if status=="warn" else "#F44336"}; color: white; padding: 6px 12px; border-radius: 20px;">{status_text}</div>'
                            f'</div>', unsafe_allow_html=True)
                ...
                # Graphique barres (2)
                st.markdown("#### Graphique des chutes par phase")
                phases = [1, 2, 3]
                drops = [p["perc_drop"] for p in r["phases"]]
                plot_voltage_drop_bar_chart(drops, phases)

                # Chutes par phase (avec mise en évidence)
                st.markdown("#### Détail par phase")
                cols = st.columns(3)
                for idx, col in enumerate(cols):
                    ph = r['phases'][idx]
                    ...
                    with col:
                        # Mise en évidence si la chute dépasse les seuils (11)
                        bg_color = "#FFEBEE" if ph['perc_drop'] > 5 else ("#FFF3E0" if ph['perc_drop'] > 3 else "#F8F9F8")
                        st.markdown(f"""
                        <div style="background:{bg_color}; border-radius:12px; padding:16px; margin:8px 0; border-left:4px solid {color};">
                ...
                # Recommandations dynamiques (6) – on combine avec les recos déjà présentes
                all_recommendations = generate_recommendations(st.session_state.res_t1, r)
                ...
                data_rows = [
                    ...
                    ("Matériau", material, ""),
                    ("cos φ", f"{Constants.COS_PHI_CHUTE}", ""),
                ...
                if REPORTLAB_AVAILABLE:
                    pdf_buffer = generate_pdf_report("Rapport Chute de Tension", data_rows, r['message'],
    ...
    <div class="onee-footer">
        <span class="badge">ONEE Tech v2.6</span>
