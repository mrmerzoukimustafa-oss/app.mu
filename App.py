...
def init_session_state():
    ...
    if "departs_charge" not in st.session_state:
        st.session_state.departs_charge = [{
            ...
            "coeff_individuel": None  # None signifie utiliser le coeff global
        }]
    # Données des départs pour la chute (inchangé)
...
def calculate_depart_contributions(depart: dict, line_voltage: float, global_coeff: float) -> Tuple[float, float, float, float]:
    """Calcule pour un départ : P, Q, S (kVA) après application du coefficient de simultanéité.
       Retourne (P_total_kW, Q_total_kVAR, S_total_kVA, cos_phi_phase)"""
...
def main():
    ...
    # (Ce code reste identique à la version précédente, je le raccourcis pour éviter de répéter)
    # ... (comme dans le message précédent) ...
    # Pour éviter de dupliquer, nous allons garder le code précédent pour l'onglet 2,
    # mais il faut noter que les départs pour la chute sont stockés séparément (departs_chute).
    # Je vais reprendre ici la version de l'onglet 2 que j'avais fournie plus tôt.
    with tab2:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
