...
# ── CSS with blue text color and gray background ───────────────────────────────
def load_css():
    st.markdown("""
    <style>
...
def generate_pdf_report(title: str, data_rows: List[Tuple[str, str, str]], summary_message: str, client_name: str = "") -> io.BytesIO:
    """Generate a PDF report with given data and optional client name."""
        ...
        with st.container():
            ...
            if st.session_state.res_t1:
                ...
                data_rows = [
                    ...
                    ("Puissance active réelle", f"{r['p_reel']:.1f}", "kW"),
                    ("Facteur de puissance", f"{r['cos_phi']:.2f}", ""),
                    ("Puissance apparente réelle", f"{r['s_reel']:.1f}", "kVA"),
                    ("Puissance réactive", f"{r['q_reel']:.1f}", "kVAR"),
                    ("Taux de charge", f"{r['charge']:.1f}", "%"),
                ]
                # Bouton PDF
    ...
    with tab2:
        with st.container():
            ...
            if st.session_state.res_t2:
                ...
                # Bouton PDF avec le nom du client
                if REPORTLAB_AVAILABLE:
