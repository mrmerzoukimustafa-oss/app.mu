...
class Constants:
    ...
    REACTANCE = 0.08  # Ω/km
    COS_PHI = 0.8                     # cos φ fixe pour la charge transformateur
...
# --- Fonction de calcul de charge (cos φ fixe 0.8) ---
def calculate_transformer_load(
    ...
    Utilise un facteur de puissance fixe = 0.8.
    """
    ...
    p_phases = []
    q_phases = []
    s_phases = []  # S par phase (apparente)
    for I in currents:
        p = phase_voltage * I * cos_phi
        q = phase_voltage * I * sin_phi
        s = phase_voltage * I
        p_phases.append(p)
        q_phases.append(q)
        s_phases.append(s)

    # Totaux avec facteur de simultanéité
    p_total = sum(p_phases) * simultaneity
    q_total = sum(q_phases) * simultaneity
    s_total = math.sqrt(p_total**2 + q_total**2)   # Puissance apparente totale (kVA)

    # Facteur de puissance global (identique au fixe)
    cos_global = cos_phi
    ...
    if future_p > 0:
        # On suppose que le facteur de puissance reste identique
...
def suggest_cable_section(s_total_kva, tension_v=400, L_m=100, material="Cuivre (Cu)"):
    ...
    cosphi = 0.8   # utiliser le cos φ de référence pour le calcul de section
...
# --- Fonctions pour la chute de tension (inchangées, avec cos φ constant 0.9) ---
# Note : pour la chute de tension on garde 0.9 car c'était demandé précédemment.
def calculate_voltage_drop_per_phase(i_phase, L, section, material, tension_ref):
    cos_phi, sin_phi = 0.9, math.sqrt(1 - 0.9**2)
    rho = Constants.RESISTIVITY[material]
    R = (rho * L) / section
...
def calculate_three_phase_unbalanced(currents, L, section, material, tension_ref):
    ...
    cos_phi_global = total_P / total_S if total_S > 0 else 0.9
    ...
    if max_drop > Constants.VOLTAGE_DROP_WARNING:
        ...
        s_min = (Constants.RESISTIVITY[material] * L * math.sqrt(3) * worst_current * 0.9) / (0.05 * tension_ref)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(f'<div class="cos-badge">cos φ charge = {Constants.COS_PHI}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="cos-badge">cos φ chute = 0.9</div>', unsafe_allow_html=True)
        st.markdown("### 📐 Formule")
        st.latex(r"\Delta U = \sqrt{3} \times I \times (R \times \cos\phi + X \times \sin\phi)")
        st.caption("cos φ = 0.9 (chute de tension)")
        ...
        with st.container():
            ...
            if st.button("Calculer la charge", key="btn_t1", use_container_width=True):
                result = calculate_transformer_load(
                    s_nom, tuple(currents), line_voltage, simultaneity, temp_amb, future_p
                )
            ...
            if st.session_state.res_t1:
                ...
                col3.metric("cos φ global", f"{r['cos_global']:.2f}")
                ...
                if cable_length > 0:
                    ...
                    st.info(f"💡 **Section de câble recommandée** (pour une chute ≤ 3% sur {cable_length:.0f} m) : **{recommended_section} mm²** (Cuivre, cosφ=0.8)")
                else:
    ...
    # ===================== TAB 2 : CHUTE DE TENSION (inchangé) =====================
    with tab2:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
            ...
            st.info(f"📐 Facteur de puissance constant: cos φ = 0.9")
    ...
    <div class="onee-footer">
        <span class="badge">ONEE Tech v2.4</span>
        <span class="badge">cos φ charge = {Constants.COS_PHI}</span>
        <span class="badge">cos φ chute = 0.9</span>
