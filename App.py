...
def init_session_state():
    ...
    if "depart_data" not in st.session_state:
        # Chaque départ est un dict avec : name, currents (list of 3), L, section, material
        st.session_state.depart_data = [{
            "name": "Départ 1",
...
# --- Fonctions de calcul pour la chute de tension (inchangées) ---
...
def calculate_depart_results(currents, L, section, material, tension_ref):
    """Calcule les résultats pour un départ donné."""
...
# --- Fonctions de calcul pour la charge (inchangées) ---
...
# --- Fonctions d'amélioration pour l'affichage ---
...
def generate_recommendations(load_result: Dict, depart_results: List[Dict]) -> List[str]:
    ...
    if depart_results:
        # On peut collecter les recos de tous les départs
...
def main():
    ...
    # ===================== TAB 1 : CHARGE TRANSFORMATEUR (inchangé) =====================
    with tab1:
        with st.container():
            ...
            if st.session_state.res_t1:
                ...
                # Indicateur de conformité clair
    ...
    with tab2:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
            st.markdown('<div class="card-title">📉 Chute de Tension - Départs BT (max 8 départs)</div>', unsafe_allow_html=True)
            ...
            # Gestion du nombre de départs
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("➕ Ajouter un départ", use_container_width=True):
                    if len(st.session_state.depart_data) < Constants.MAX_DEPARTS:
                        st.session_state.depart_data.append({
                            "name": f"Départ {len(st.session_state.depart_data)+1}",
            ...
            with col_btn2:
                if st.button("➖ Supprimer le dernier départ", use_container_width=True):
                    if len(st.session_state.depart_data) > 1:
                        st.session_state.depart_data.pop()
                    else:
                        st.warning("Il doit y avoir au moins un départ.")

            # Affichage des départs
            depart_results = []
            for i, depart in enumerate(st.session_state.depart_data):
                with st.expander(f"📌 {depart['name']} (Départ {i+1})", expanded=(i==0)):
                    col1, col2 = st.columns([2, 1])
                    with col1:
                        new_name = st.text_input("Nom du départ", value=depart["name"], key=f"name_{i}")
                        st.session_state.depart_data[i]["name"] = new_name
                    with col2:
                        st.markdown("**Courants (A)**")
                        cols_c = st.columns(3)
                        currents = [depart["currents"][0], depart["currents"][1], depart["currents"][2]]
                        for j, col_c in enumerate(cols_c):
                            with col_c:
                                new_cur = st.number_input(f"Phase {j+1}", value=currents[j], step=5.0, key=f"cur_{i}_{j}", format="%.1f")
                                currents[j] = new_cur
                        st.session_state.depart_data[i]["currents"] = currents
                    colL, colS, colM = st.columns(3)
                    with colL:
                        L = st.number_input("Longueur (m)", value=depart["L"], step=10.0, key=f"L_{i}")
                        st.session_state.depart_data[i]["L"] = L
                    with colS:
                        section = st.selectbox("Section (mm²)", Constants.CABLE_SECTIONS, index=Constants.CABLE_SECTIONS.index(depart["section"]), key=f"sec_{i}")
                        st.session_state.depart_data[i]["section"] = section
                    with colM:
                        material = st.selectbox("Matériau", list(Constants.RESISTIVITY.keys()), index=0 if depart["material"]=="Cuivre (Cu)" else 1, key=f"mat_{i}")
                        st.session_state.depart_data[i]["material"] = material

            # Bouton de calcul global
            if st.button("Calculer tous les départs", key="btn_t2", use_container_width=True):
                depart_results.clear()
                for depart in st.session_state.depart_data:
            ...
            if st.session_state.depart_results:
                ...
                # Recommandations globales (incluant charge et départs)
                ...
                # Export PDF pour les départs (optionnel)
                if REPORTLAB_AVAILABLE:
                    # Préparation des données pour un rapport global
                    all_data_rows = [("Tension source", f"{tension_ref:.0f}", "V")]
                    for idx, depart in enumerate(st.session_state.depart_data):
    ...
    <div class="onee-footer">
        <span class="badge">ONEE Tech v3.0</span>
