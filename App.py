...
def init_session_state():
    ...
    # Données des départs pour la charge
    ...
    # Données des départs pour la chute
    ...
    if "anomalies" not in st.session_state:
        st.session_state.anomalies = {
            ...
            "composants": {
                "transformateur_bruit": False,
                "transformateur_echauffement": False,
                "jeu_barres": False,
                "disjoncteurs": False,
                "parafoudres": False,
                "cables": False,
...
# --- PDF Generation (avec anomalies) ---
def generate_pdf_report(title: str, data_rows: List[Tuple[str, str, str]], summary_message: str, client_name: str = "", poste_name: str = "", poste_matricule: str = "", anomalies: dict = None) -> io.BytesIO:
    ...
    # Ajout des anomalies si présentes
    if anomalies:
        table_data.append(["", "", ""])
        table_data.append(["Anomalies constatées", "", ""])
        # Bâtiment
        bat = anomalies.get("batiment", {})
        bat_items = []
        if bat.get("infiltration"): bat_items.append("Infiltration d'eau")
        if bat.get("proprete"): bat_items.append("Manque de propreté")
        if bat.get("eclairage"): bat_items.append("Éclairage défectueux")
        if bat.get("ventilation"): bat_items.append("Ventilation insuffisante")
        if bat.get("acces"): bat_items.append("Accès difficile")
        if bat_items:
            table_data.append(["  Bâtiment", ", ".join(bat_items), ""])
        # Composants
        comp = anomalies.get("composants", {})
        comp_items = []
        if comp.get("transformateur_bruit"): comp_items.append("Transformateur bruyant")
        if comp.get("transformateur_echauffement"): comp_items.append("Transformateur anormalement chaud")
        if comp.get("jeu_barres"): comp_items.append("Jeu de barres anormal")
        if comp.get("disjoncteurs"): comp_items.append("Disjoncteurs défectueux")
        if comp.get("parafoudres"): comp_items.append("Parafoudres endommagés")
        if comp.get("cables"): comp_items.append("Câbles dégradés")
        if comp.get("relais"): comp_items.append("Relais défaillants")
        if comp_items:
            table_data.append(["  Composants", ", ".join(comp_items), ""])
        # Observations
        obs = anomalies.get("observations", "")
        if obs:
            table_data.append(["Observations", obs, ""])
...
def main():
    ...
    with tab1:
        with st.container():
            ...
            # ========== NOUVEAU : Checklist des anomalies ==========
            st.markdown("---")
            st.markdown("### 🔍 Inspection visuelle")
            with st.expander("Liste des anomalies (bâtiment et composants)", expanded=False):
                st.markdown("#### Bâtiment")
                col_b1, col_b2, col_b3 = st.columns(3)
                with col_b1:
                    st.session_state.anomalies["batiment"]["infiltration"] = st.checkbox("Infiltration d'eau", value=st.session_state.anomalies["batiment"]["infiltration"], key="infiltration")
                    st.session_state.anomalies["batiment"]["proprete"] = st.checkbox("Manque de propreté", value=st.session_state.anomalies["batiment"]["proprete"], key="proprete")
                with col_b2:
                    st.session_state.anomalies["batiment"]["eclairage"] = st.checkbox("Éclairage défectueux", value=st.session_state.anomalies["batiment"]["eclairage"], key="eclairage")
                    st.session_state.anomalies["batiment"]["ventilation"] = st.checkbox("Ventilation insuffisante", value=st.session_state.anomalies["batiment"]["ventilation"], key="ventilation")
                with col_b3:
                    st.session_state.anomalies["batiment"]["acces"] = st.checkbox("Accès difficile", value=st.session_state.anomalies["batiment"]["acces"], key="acces")

                st.markdown("#### Composants électriques")
                col_c1, col_c2, col_c3 = st.columns(3)
                with col_c1:
                    st.session_state.anomalies["composants"]["transformateur_bruit"] = st.checkbox("Transformateur bruyant", value=st.session_state.anomalies["composants"]["transformateur_bruit"], key="trans_bruit")
                    st.session_state.anomalies["composants"]["transformateur_echauffement"] = st.checkbox("Transformateur anormalement chaud", value=st.session_state.anomalies["composants"]["transformateur_echauffement"], key="trans_chaud")
                with col_c2:
                    st.session_state.anomalies["composants"]["jeu_barres"] = st.checkbox("Jeu de barres anormal", value=st.session_state.anomalies["composants"]["jeu_barres"], key="jeu_barres")
                    st.session_state.anomalies["composants"]["disjoncteurs"] = st.checkbox("Disjoncteurs défectueux", value=st.session_state.anomalies["composants"]["disjoncteurs"], key="disjoncteurs")
                with col_c3:
                    st.session_state.anomalies["composants"]["parafoudres"] = st.checkbox("Parafoudres endommagés", value=st.session_state.anomalies["composants"]["parafoudres"], key="parafoudres")
                    st.session_state.anomalies["composants"]["cables"] = st.checkbox("Câbles dégradés", value=st.session_state.anomalies["composants"]["cables"], key="cables")
                    st.session_state.anomalies["composants"]["relais"] = st.checkbox("Relais défaillants", value=st.session_state.anomalies["composants"]["relais"], key="relais")

                st.markdown("#### Observations complémentaires")
                st.session_state.anomalies["observations"] = st.text_area("Observations", value=st.session_state.anomalies["observations"], height=100, key="obs_anomalies")
            ...
            if st.session_state.res_t1:
                ...
                # Export PDF pour la charge (avec anomalies)
                data_rows = [
    ...
    # ===================== TAB 2 : CHUTE DE TENSION - DÉPARTS BT (inchangé) =====================
    with tab2:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
    ...
    <div class="onee-footer">
        <span class="badge">Tech Assistant v3.3</span>
