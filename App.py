...
class Constants:
    ...
    REACTANCE = 0.08  # Ω/km
    COS_PHI_CHARGE = 0.8                     # cos φ pour la charge transformateur
    SIN_PHI_CHARGE = math.sqrt(1 - COS_PHI_CHARGE**2)
    COS_PHI_CHUTE = 0.9                      # cos φ pour la chute de tension
...
# --- CSS (inchangé, mais conservé pour l'affichage) ---
def load_css():
    st.markdown("""
    <style>
...
# --- Fonction de calcul de charge (cos φ fixe 0.8, puissances en kW/kVAR/kVA) ---
def calculate_transformer_load(
    ...
    phase_voltage = line_voltage / math.sqrt(3)   # tension simple (V)

    # Calcul des puissances par phase (en kW, kVAR, kVA)
    p_phases = []
    q_phases = []
    s_phases = []
    for I in currents:
        p = (phase_voltage * I * cos_phi) / 1000   # kW
        q = (phase_voltage * I * sin_phi) / 1000   # kVAR
        s = (phase_voltage * I) / 1000             # kVA
        p_phases.append(p)
        q_phases.append(q)
        s_phases.append(s)
    ...
    s_total = math.sqrt(p_total**2 + q_total**2)   # kVA

    cos_global = cos_phi   # global identique au fixe
...
def suggest_cable_section(s_total_kva, tension_v=400, L_m=100, material="Cuivre (Cu)"):
    ...
    cosphi = Constants.COS_PHI_CHARGE  # on utilise le cos φ de la charge pour la recommandation
...
# --- Fonctions pour la chute de tension (cos φ fixe 0.9) ---
...
def main():
    ...
    <div class="onee-footer">
        <span class="badge">ONEE Tech v2.5</span>
