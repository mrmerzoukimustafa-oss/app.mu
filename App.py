...
def calculate_depart_contributions(
    depart: Dict[str, Any],
    line_voltage: float,
    global_coeff: float,
    ...
    for current in depart["currents"]:
        p = (phase_voltage * current * cos_phi) / 1000
        q = (phase_voltage * current * sin_phi) / 1000
        s = (phase_voltage * current) / 1000
        p_phases.append(p)
        q_phases.append(q)
        s_phases.append(s)
...
def calculate_transformer_load_multi_depart(
    ...
    for dep in departs:
        p_dep, q_dep, s_dep, cos_dep, p_ph, q_ph, s_ph = calculate_depart_contributions(
            dep, line_voltage, global_simultaneity
        )
        p_total_all += p_dep
        q_total_all += q_dep
        details.append(
    ...
    if temp_amb > 40:
        derate_factor = max(0.0, 1 - Constants.TEMP_DERATE_COEFF * (temp_amb - 40))
        s_nom_derated = s_nom * derate_factor
    else:
        derate_factor = 1.0
        s_nom_derated = s_nom

    charge_actual = (s_total / s_nom_derated) * 100 if s_nom_derated > 0 else 0.0

    if future_p > 0:
        s_future = future_p / Constants.COS_PHI_CHARGE
        charge_future = (s_future / s_nom_derated) * 100 if s_nom_derated > 0 else 0.0
    else:
        future_p = 0.0
        charge_future = 0.0
    ...
    if charge_future > 100 and future_p > 0:
        message_future = (
            f"⚠️ Attention : avec la puissance prévue ({future_p:.1f} kW), "
            f"le taux de charge atteindra {charge_future:.1f}%."
        )
...
def suggest_cable_section(
    s_total_kva: float,
    tension_v: float = 400,
    L_m: float = 100,
    material: str = "Cuivre (Cu)",
) -> int:
    if tension_v <= 0:
        return Constants.CABLE_SECTIONS[0]

    current = (s_total_kva * 1000) / (math.sqrt(3) * tension_v)
    delta_u_max = 0.03 * tension_v
    rho = Constants.RESISTIVITY[material]
    cosphi = Constants.COS_PHI_CHARGE
    s_min = (math.sqrt(3) * current * rho * L_m * cosphi) / delta_u_max if delta_u_max > 0 else 0

    for section in Constants.CABLE_SECTIONS:
        if section >= s_min:
            return section
    return Constants.CABLE_SECTIONS[-1]


# --- Fonctions de calcul pour la chute de tension ---
def calculate_voltage_drop_per_phase(
    ...
    perc_drop = (delta_u / tension_ref) * 100 if tension_ref > 0 else 0.0
...
def calculate_depart_results(
    ...
    i_avg = sum(currents) / 3 if currents else 0.0
    ...
    if i_avg > 0:
        ...
        max_dev, deviations, imb_status, imb_msg = 0.0, [0.0, 0.0, 0.0], "ok", "Aucun courant"

    phases = []
    for idx, current in enumerate(currents):
        res = calculate_voltage_drop_per_phase(current, L, section, material, tension_ref)
        res["phase"] = idx + 1
        res["current"] = current
        phases.append(res)
    ...
    avg_drop = sum(drops) / 3 if drops else 0.0
    max_drop = max(drops) if drops else 0.0
    min_drop = min(drops) if drops else 0.0
    max_phase = phases[drops.index(max_drop)]["phase"] if drops else 1
    ...
    if max_drop > Constants.VOLTAGE_DROP_WARNING and drops:
        worst_idx = drops.index(max_drop)
        worst_current = currents[worst_idx]
        s_min = (
            Constants.RESISTIVITY[material]
            * L
            * math.sqrt(3)
            * worst_current
            * Constants.COS_PHI_CHUTE
        ) / (0.05 * tension_ref) if tension_ref > 0 else 0
...
def get_load_status(charge: float) -> Tuple[str, str]:
    ...
    if charge > Constants.CHARGE_WARNING:
...
def get_drop_status(drop: float) -> Tuple[str, str]:
    ...
    if drop > Constants.VOLTAGE_DROP_WARNING:
        return "warn", "⚠️ Limite (3-5%)"
    return "ok", "✅ Conforme (≤3%)"


def get_phase_color(drop: float) -> str:
    if drop > Constants.VOLTAGE_DROP_CRITICAL:
        return "#D32F2F"
    if drop > Constants.VOLTAGE_DROP_WARNING:
        return "#FF9800"
...
def get_phase_bg_color(drop: float) -> str:
    if drop > Constants.VOLTAGE_DROP_CRITICAL:
        return "#FFEBEE"
    if drop > Constants.VOLTAGE_DROP_WARNING:
        return "#FFF3E0"
...
def plot_voltage_drop_bar_chart(drops: List[float], phases: List[int], title: str = "Chutes de tension") -> None:
    fig, ax = plt.subplots(figsize=(6, 4))
    colors_bar = [get_phase_color(drop) for drop in drops]
    bars = ax.bar(phases, drops, color=colors_bar)
    ...
    ax.set_xlabel("Phase")
    ax.set_title(title)
    ax.set_ylim(0, max(max(drops) if drops else 0, 5.5))
    ax.legend()
...
def generate_recommendations(
    ...
    depart_results: List[Dict[str, Any]],
) -> List[str]:
    recs: List[str] = []
    ...
    if load_result:
        ...
        if charge > Constants.CHARGE_CRITICAL:
            recs.append(
                "🔧 SURCHARGE TRANSFORMATEUR : Remplacez par un transformateur de puissance supérieure ou réduisez la charge."
            )
        ...
        if load_result["charge_future"] > 100 and load_result["future_p"] > 0:
            recs.append(
                f"📈 ÉVOLUTION FUTURE : Avec la puissance prévue ({load_result['future_p']:.0f} kW), "
                "le transformateur sera en surcharge. Anticipez un remplacement."
            )
    ...
    if depart_results:
        for idx, res in enumerate(depart_results):
            ...
            if res["imbalance"]["max_deviation"] > Constants.UNBALANCE_WARNING:
                recs.append(
                    f"⚖️ DÉPART {idx + 1} : Déséquilibre de {res['imbalance']['max_deviation']:.1f}%. Répartissez les charges."
                )
            if res["max_drop"] > Constants.VOLTAGE_DROP_CRITICAL:
                recs.append(
                    f"📉 DÉPART {idx + 1} : Chute excessive ({res['max_drop']:.2f}%). Augmentez la section du câble ou réduisez la distance."
                )
            elif res["max_drop"] > Constants.VOLTAGE_DROP_WARNING:
                recs.append(
                    f"⚠️ DÉPART {idx + 1} : Chute limite ({res['max_drop']:.2f}%). Vérifiez le dimensionnement."
                )
...
def main() -> None:
    st.set_page_config(page_title="ONEE Tech Assistant", page_icon="⚡", layout="wide")
    init_session_state()
    load_css()
    ...
    with st.sidebar:
        ...
        st.markdown(
            f'<div class="cos-badge">cos φ charge = {Constants.COS_PHI_CHARGE}</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f'<div class="cos-badge">cos φ chute = {Constants.COS_PHI_CHUTE}</div>',
            unsafe_allow_html=True,
        )
        st.markdown("### 📐 Formule")
        ...
        if st.session_state.res_t1:
            ...
            if ch > Constants.CHARGE_WARNING:
                st.warning(f"Charge: {ch:.1f}%")
            else:
                st.success(f"Charge: {ch:.1f}%")

        if st.session_state.res_t2:
            md = st.session_state.res_t2["max_drop"]
            if md > Constants.VOLTAGE_DROP_CRITICAL:
                st.error(f"Chute max: {md:.2f}%")
            elif md > Constants.VOLTAGE_DROP_WARNING:
                st.warning(f"Chute max: {md:.2f}%")
            else:
                st.success(f"Chute max: {md:.2f}%")
    ...
    with tab1:
        with st.container():
            ...
            st.markdown(
                '<div class="card-title">🔌 Charge Transformateur - Multi départs BT (max 8)</div>',
                unsafe_allow_html=True,
            )

            col_id1, col_id2 = st.columns(2)
            with col_id1:
                poste_name = st.text_input(
                    "Nom du poste",
                    value=st.session_state.poste_name,
                    key="poste_name_input",
                )
                st.session_state.poste_name = poste_name
            with col_id2:
                poste_matricule = st.text_input(
                    "Matricule du poste",
                    value=st.session_state.poste_matricule,
                    key="poste_matricule_input",
                )
            ...
            global_simultaneity = st.slider(
                "Facteur de simultanéité global",
                0.5,
                1.0,
                1.0,
                0.01,
                help="Appliqué à tous les départs qui n'ont pas de coefficient individuel.",
            )
            ...
            for i, depart in enumerate(st.session_state.departs_charge):
                with st.expander(f"📌 {depart['name']} (Départ {i + 1})", expanded=False):
                    ...
                    with col_coeff:
                        coeff_val = (
                            depart["coeff_individuel"] if depart["coeff_individuel"] is not None else global_simultaneity
                        )
                        use_indiv = st.checkbox(
                            "Utiliser un coefficient individuel",
                            value=(depart["coeff_individuel"] is not None),
                            key=f"use_indiv_{i}",
                        )
                        if use_indiv:
                    ...
                    for j, col_c in enumerate(cols_c):
                        with col_c:
                            new_cur = st.number_input(
                                f"Phase {j + 1}",
                                min_value=0.0,
                                value=float(currents[j]),
                                step=5.0,
                                key=f"charge_cur_{i}_{j}",
                                format="%.1f",
                            )
                            currents[j] = new_cur
                    st.session_state.departs_charge[i]["currents"] = currents

            future_p = st.number_input(
                "Puissance active prévue dans 5 ans (kW) - optionnel",
                min_value=0.0,
                value=0.0,
                step=10.0,
            )
            cable_length = st.number_input(
                "Longueur du câble BT (m) - pour recommandation de section",
                min_value=0.0,
                value=100.0,
                step=10.0,
            )
            ...
            if st.session_state.res_t1:
                ...
                status_color = "#4CAF50" if status == "ok" else "#FF9800" if status == "warn" else "#F44336"
                st.markdown(
                    f'<div style="display: flex; justify-content: space-between; align-items: center;">'
                    f'<div><span style="font-size: 24px; font-weight: bold;">Taux de charge</span></div>'
                    f'<div style="background: {status_color}; color: white; padding: 6px 12px; border-radius: 20px;">{status_text}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
                ...
                for dep in r["details"]:
                    with st.expander(f"{dep['name']} - P: {dep['p']:.1f} kW, S: {dep['s']:.1f} kVA", expanded=False):
                        st.markdown(
                            f"**Courants :** {dep['currents'][0]:.1f} A / {dep['currents'][1]:.1f} A / {dep['currents'][2]:.1f} A"
                        )
                        ...
                        for j, col_ph in enumerate(cols_ph):
                            with col_ph:
                                st.markdown(
                                    (
                                        f"Phase {j + 1}<br/>"
                                        f"P: {dep['p_phases'][j]:.2f} kW<br/>"
                                        f"Q: {dep['q_phases'][j]:.2f} kVAR<br/>"
                                        f"S: {dep['s_phases'][j]:.2f} kVA"
                                    ),
                                    unsafe_allow_html=True,
                                )

                if cable_length > 0:
                    recommended_section = suggest_cable_section(
                        r["s_total"],
                        tension_v=line_voltage,
                        L_m=cable_length,
                        material="Cuivre (Cu)",
                    )
                    st.info(
                        f"💡 **Section de câble recommandée** (pour une chute ≤ 3% sur {cable_length:.0f} m) : "
                        f"**{recommended_section} mm²** (Cuivre, cosφ={Constants.COS_PHI_CHARGE})"
                    )
                else:
                ...
                if r["charge_actual"] > Constants.CHARGE_WARNING:
                    recommendation = (
                        "Augmenter la puissance du transformateur ou réduire la charge."
                        if r["charge_actual"] > Constants.CHARGE_CRITICAL
                        else "Surveiller l'évolution de la charge."
                    )
                ...
                for idx, dep in enumerate(r["details"]):
                    data_rows.append((f"Départ {idx + 1} - {dep['name']}", "", ""))
                    data_rows.append(("  Courants", f"{dep['currents'][0]:.1f}/{dep['currents'][1]:.1f}/{dep['currents'][2]:.1f}", "A"))
                    data_rows.append(("  Coefficient appliqué", f"{dep['coeff_applique']:.2f}", ""))
                    data_rows.append(("  P", f"{dep['p']:.1f}", "kW"))
                    data_rows.append(("  Q", f"{dep['q']:.1f}", "kVAR"))
                    data_rows.append(("  S", f"{dep['s']:.1f}", "kVA"))

                if REPORTLAB_AVAILABLE:
                    pdf_buffer = generate_pdf_report(
                        "Rapport Charge Transformateur - Multi départs",
                        data_rows,
                        r["message"],
                        client_name=st.session_state.client_name,
                        poste_name=poste_name,
                        poste_matricule=poste_matricule,
                    )
                    if pdf_buffer:
                        st.download_button(
    ...
    with tab2:
        with st.container():
            st.markdown('<div class="calc-card">', unsafe_allow_html=True)
            st.markdown(
                '<div class="card-title">📉 Chute de Tension - Départs BT (max 8)</div>',
                unsafe_allow_html=True,
            )
                ...
                with st.expander(f"📌 {depart['name']} (Départ {i + 1})", expanded=False):
                    ...
                    with colS:
                        section = st.selectbox(
                            "Section (mm²)",
                            Constants.CABLE_SECTIONS,
                            index=Constants.CABLE_SECTIONS.index(depart["section"]),
                            key=f"chute_sec_{i}",
                        )
                    ...
                    with colM:
                        material = st.selectbox(
                            "Matériau",
                            list(Constants.RESISTIVITY.keys()),
                            index=0 if depart["material"] == "Cuivre (Cu)" else 1,
                            key=f"chute_mat_{i}",
                        )
            ...
            if st.button("Calculer tous les départs", key="btn_t2", use_container_width=True):
                ...
                st.session_state.res_t2 = {
                    "max_drop": max([r["max_drop"] for r in depart_results]) if depart_results else 0.0
                }

            if "depart_results" in st.session_state and st.session_state.depart_results:
                st.markdown("---")
                st.markdown("### Résultats par départ")
                critical_depart = None
                max_drop_overall = 0.0
                ...
                for idx, res in enumerate(st.session_state.depart_results):
                    ...
                    with st.expander(f"📊 {depart_name} - Chute max: {drop_max:.2f}%", expanded=False):
                        ...
                        status_color = "#4CAF50" if status == "ok" else "#FF9800" if status == "warn" else "#F44336"
                        st.markdown(
                            f'<div style="background: {status_color}; color: white; padding: 4px 8px; border-radius: 12px; display: inline-block; margin-bottom: 12px;">{status_text}</div>',
                            unsafe_allow_html=True,
                        )
                        ...
                        for ph_idx, col in enumerate(cols):
                            ...
                            color = get_phase_color(ph["perc_drop"])
                            bg_color = get_phase_bg_color(ph["perc_drop"])
                            with col:
                                st.markdown(
                                    f"""
                ...
                if REPORTLAB_AVAILABLE:
                    all_data_rows: List[Tuple[str, str, str]] = [("Tension source", f"{tension_ref:.0f}", "V")]
                    for idx, depart in enumerate(st.session_state.departs_chute):
                        ...
                        all_data_rows.append(("  Longueur", f"{depart['L']:.1f}", "m"))
                        all_data_rows.append(("  Section", f"{depart['section']}", "mm²"))
                        all_data_rows.append(("  Matériau", depart["material"], ""))

                    summary_msg = f"Départ critique: {critical_depart} avec {max_drop_overall:.2f}% de chute."
                    pdf_buffer = generate_pdf_report(
                        "Rapport Chute de Tension - Départs BT",
                        all_data_rows,
                        summary_msg,
                        client_name=st.session_state.client_name,
                        poste_name=st.session_state.poste_name,
                        poste_matricule=st.session_state.poste_matricule,
                    )
                    if pdf_buffer:
                        st.download_button(
    ...
    st.markdown(
        ...
        <div class="onee-footer">
            <span class="badge">ONEE Tech v3.1</span>
